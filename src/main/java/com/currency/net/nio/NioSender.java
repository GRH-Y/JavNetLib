...
public class NioSender extends AbsNetSender {
    ...
    public void clear() {
        for (Object obj : mDataQueue) {
            if (obj instanceof ReuseDirectBuf) {
                ReuseDirectBuf buf = (ReuseDirectBuf) obj;
    ...
    protected Object onCheckAndChangeData(Object objData) {
        if (objData instanceof byte[]) {
            ...
            return null;
        } else if (objData instanceof ReuseDirectBuf) {
            ReuseDirectBuf buf = (ReuseDirectBuf) objData;
            if (buf.isHasData()) {
                return buf;
            } else {
                XMultiplexCacheManger.getInstance().lose(buf);
    ...
    protected int onHandleSendData(Object data) throws Throwable {
        ...
        } else if (data instanceof ReuseDirectBuf) {
            ReuseDirectBuf buf = (ReuseDirectBuf) data;
